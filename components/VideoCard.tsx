...
const VideoCard: React.FC<VideoCardProps> = ({ video, onDownload, onDelete }) => {
  ...
  const getThumbnailUrl = useCallback((publicId: string) => getCldImageUrl({
    src: publicId,
    width: 400,
    height: 225,
    crop: "fill",
  ...
  const getFullVideoUrl = useCallback((publicId: string) => getCldVideoUrl({
    src: publicId,
    width: 1920,
    height: 1080,
  }), []);

  const getPreviewVideoUrl = useCallback((publicId: string) => getCldVideoUrl({
    src: publicId,
    width: 400,
    height: 225,
  ...
  const handleDelete = async (publicId: string) => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`/api/videos?publicId=${publicId}`, { method: "DELETE" });
    ...
    >
      <figure className="aspect-video relative">
        ...
        <div className="absolute bottom-2 right-2 bg-base-100 bg-opacity-70 px-2 py-1 rounded-lg text-sm flex items-center">
          <Clock size={16} className="mr-1" /> {formatDuration(video.duration)}
        </div>
      </figure>
